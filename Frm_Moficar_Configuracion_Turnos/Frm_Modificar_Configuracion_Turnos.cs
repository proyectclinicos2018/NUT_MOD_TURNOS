...
{
    public partial class Frm_Modificar_Configuracion_Turnos : Form
    {
        ...
        #endregion
        public Frm_Modificar_Configuracion_Turnos()
        {
            InitializeComponent();
        }

        private void Frm_Modificar_Configuracion_Turnos_Load(object sender, EventArgs e)
        {
            conectar();
        ...
        protected void agregarimagen()
        {
            foreach (DataGridViewRow row in grilla_turnos.Rows)
            {
                ...
                string ve = Convert.ToString(row.Cells["VIGENCIAS"].Value);
                DataGridViewImageCell Imagen = row.Cells["V"] as DataGridViewImageCell;

                if (ve == "S")
                {
                    Imagen.Value = (System.Drawing.Image)Modificar_Configuracion_Turnos.Properties.Resources.Check;
                    
                }
                else
                {
                    Imagen.Value = (System.Drawing.Image)Modificar_Configuracion_Turnos.Properties.Resources.Delete;
        ...
        void ocultar_grilla_turnos()
        {
            ...
            grilla_turnos.Columns["VIGENCIAS"].Visible = false;
        ...
        private void grilla_turnos_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (e.RowIndex < 0)
            {
                ...
                e.Graphics.DrawImage(Modificar_Configuracion_Turnos.Properties.Resources.HeaderGV, e.CellBounds);
        ...
        private void grilla_turnos_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == 0)
                {
                    DialogResult opc = MessageBox.Show("Estimado usuario, Esta seguro de Cambiar la Vigencia de Cama", "Informacion", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (opc == DialogResult.Yes)
                    {
                        int id_turno = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["ID_TURNOS"].Value.ToString());
                        vigencia =grilla_turnos.Rows[e.RowIndex].Cells["VIGENCIAS"].Value.ToString();
                        int c_camas = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["COD_CAMAS"].Value.ToString());


                       
                            foreach (DataRow fila in dt_turnos.Select(" COD_CAMAS= '" + c_camas+ "'"))
                            {
                               string v = fila["VIGENCIA"].ToString();
                ...
                if (e.ColumnIndex == 1)
                {
                    id_turno = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["ID_TURNOS"].Value.ToString());
                    cod_turno = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["COD_TURNOS"].Value.ToString());
                    foreach (DataRow fila in dt_turnos.Select(" COD_TURNOS= '" + cod_turno + "'"))
                    {
                        string nom = fila["NOM_TURNOS"].ToString();
                        if(nom!="")
                        {
                            txttipo_turno.Text=nom;
                        }
                    }
                   
                    cod_habitacion = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["COD_HABITACIONS"].Value.ToString());
                    txtnum_habitacion.Text = grilla_turnos.Rows[e.RowIndex].Cells["NOM_HABITACIONS"].Value.ToString();
                   cod_cama = Convert.ToInt32(grilla_turnos.Rows[e.RowIndex].Cells["COD_CAMAS"].Value.ToString());
                   txtnum_cama.Text = grilla_turnos.Rows[e.RowIndex].Cells["NUM_CAMAS"].Value.ToString();
                   vigencia =grilla_turnos.Rows[e.RowIndex].Cells["VIGENCIAS"].Value.ToString();
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Estimado Usuario, No existen datos en grilla", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
